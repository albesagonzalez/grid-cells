function m = movie_maker(filename)
...
S = load(filename).S;
original_frames = length(S(:,1));
N = length(S(1, :))
n = sqrt(N);

x1d = linspace(-3, 3, original_frames);
y1d = x1d;
t = linspace(0, 5, original_frames);
hFigure = figure;

% Set up the movie structure.
% Preallocate movie, which will be an array of structures.
% First get a cell array with all the frames.
allTheFrames = cell(original_frames,1);
vidHeight = 344;
vidWidth = 446;
allTheFrames(:) = {zeros(vidHeight, vidWidth, 3, 'uint8')};
% Next get a cell array with all the colormaps.
allTheColorMaps = cell(original_frames,1);
...
for frameIndex = 1 : original_frames
%%    
	Z = reshape(S(frameIndex,:),[n, n]);
	cla reset;
...
%     [max, position] = maxk(S(frameIndex,:), 500);
...
%%    
	caption = sprintf('Frame #%d of %d, t = %.1f', frameIndex, original_frames, t(frameIndex));
